...
def create_index(output_folder: str) -> MultiModalVectorStoreIndex:
    text_store = MilvusVectorStore(uri="http://127.0.0.1:19530", collection_name="text_collection", overwrite=True, dim=1024)
...
def retrieve(retriever_engine, query_str) -> tuple[list[str], list[str]]:
    ...
    retrieved_image = [res_node.node.metadata["file_path"] for res_node in retrieval_results if isinstance(res_node.node, ImageNode)]
    retrieved_text = [res_node.text for res_node in retrieval_results if not isinstance(res_node.node, ImageNode)]
    return retrieved_image, retrieved_text

def process_query_with_image(query_str, context_str, metadata_str, image_document) -> str | None:
    client = OpenAI(base_url=os.getenv("KOYEB_ENDPOINT"), api_key=os.getenv("KOYEB_TOKEN"))
    ...
    messages = [
        {
            "role": "user",
            "content": [
                {"type": "text", "text": qa_tmpl_str.format(context_str=context_str, query_str=query_str, metadata_str=metadata_str)},
...
def main():
    ...
    if video_path and not st.session_state.index:
        ...
        with st.spinner("Processing video..."):
            st.session_state.metadata = process_video(video_path, output_folder, output_audio_path)
    ...
    if st.session_state.index:
        ...
        if query:
            with st.spinner("Generating response..."):
                ...
                image_documents = SimpleDirectoryReader(input_dir="./mixed_data/", input_files=img).load_data()
                context_str = "".join(txt)
                metadata_str = json.dumps(st.session_state.metadata)
                
                response = process_query_with_image(query, context_str, metadata_str, image_documents[0])
                st.write("Response:", response)
                
                # Display the images used in the explanation
                st.subheader("Relevant Images:")
                cols = st.columns(min(3, len(image_documents)))  # Create up to 3 columns
                for idx, image_doc in enumerate(image_documents):
                    with cols[idx % 3]:
                        st.image(image_doc.image_path, caption=f"Image {idx + 1}", use_column_width=True)
    elif video_path:
